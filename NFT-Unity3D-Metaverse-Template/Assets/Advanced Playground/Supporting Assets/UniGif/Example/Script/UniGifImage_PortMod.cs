...
/// </summary>
public class UniGifImage_PortMod : MonoBehaviour
{
    /// <summary>
    ...
    // Target row image
    public List<RawImage> m_rawImage = new List<RawImage>();
    ...
    private void Update()
    {
        switch (nowState)
        {
            ...
            case State.Playing:
                ...
                foreach(var rawImag in m_rawImage)
                {
                    rawImag.texture = m_gifTextureList[m_gifTextureIndex].m_texture2d;
                }
    ...
    public UniGifImage_PortMod SetGifFromUrl(string url, bool autoPlay = true)
    {
        StartCoroutine(SetGifFromUrlCoroutine(url, autoPlay));
        return this;
    }

    private List<Nft> NFtsUsingThis = new List<Nft>();
    public UniGifImage_PortMod AddNft(Nft nft)
    {
        NFtsUsingThis.Add(nft);
        return this;
    }
    
    
    private UnityAction<Nft, UniGifImage_PortMod> OnCompleteAction;
    private Nft nft;
    /// <summary>
    /// Action on succes ≧◉◡◉≦.
    /// </summary>
    /// <returns> Storage_model</returns>
    public UniGifImage_PortMod OnComplete(Nft nft,UnityAction<Nft , UniGifImage_PortMod> action)
    {
        this.nft = nft;
    ...
    private float autokillAfter = 7f;
    public UniGifImage_PortMod AutoKillIfLongDownload(float autokillAfter = 7f)
    {
        this.autokillAfter = autokillAfter;
        StartCoroutine(AutoKillIfBigDL());
        return this;
    }
    ...
    /// <returns> Information on Error as string text and this UniGif.</returns>
    private UnityAction<string, UniGifImage_PortMod> OnErrorAction;
    public UniGifImage_PortMod OnError(UnityAction<string, UniGifImage_PortMod> action)
    {
        this.OnErrorAction = action;
        return this;
    }

    IEnumerator AutoKillIfBigDL()
    {
        yield return new WaitForSeconds(autokillAfter);
        ...
        if (nowState == State.Loading)
        {
            ...
            if(OnErrorAction!=null)
                OnErrorAction.Invoke("AutoKill", this);
    ...
    public IEnumerator SetGifFromUrlCoroutine(string url, bool autoPlay = true)
    {
        ...
        // Load file
        using (www = new UnityWebRequest(path, "GET"))
        {
            ...
            if (string.IsNullOrEmpty(www.error) == false)
            {
                ...
                if (OnErrorAction != null)
                    OnErrorAction(
                        $"Null data. Response code: {www.responseCode.ToString()}. Result {www.downloadHandler.text}", this);
                yield break;
            }
            ...
            yield return StartCoroutine(UniGif.GetTextureListCoroutine(www.downloadHandler.data, (gifTexList, loopCount, width, height) =>
            {
                if (gifTexList != null)
                {
                   ...
                   // m_imgAspectCtrl.FixAspectRatio(width, height);
                    ...
                    foreach (var NFtUsingThis in NFtsUsingThis)
                    {
                        if (OnCompleteAction != null)
                        {
                            OnCompleteAction.Invoke(NFtUsingThis, this);
                        }
                    }
                }
                else
    ...
    public void Play()
    {
        ...
        foreach (var rawImag in m_rawImage)
        {
            rawImag.texture = m_gifTextureList[0].m_texture2d;
        }
